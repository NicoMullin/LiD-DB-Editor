...
def write_version_file(folder: Path) -> Path:
    ...
    PyInstaller build has none. The version comes from the package, so it
    cannot drift from what the app reports about itself.
    """
    from lid_db_manager import APP_NAME as PRODUCT, __version__
...
"""
    ...
    print(f"Version info: {PRODUCT} {__version__}, KSFA")
    return path
...
def bootloader_note() -> None:
    ...
    The bootloader is the small exe PyInstaller wraps around every app, and the
    one in PyInstaller's wheel is byte-identical in every PyInstaller program -
    malware included - which is why small antivirus engines flag it on sight
    ("Dropper.Agent" and the like). Compiled from source it gets its own hash.
    A local compile links it minutes before the package is installed; the stock
    one was linked when PyInstaller was released, long before.
    """
    import struct
...
# replaced - plus the list of which game package holds which texture, which
# takes minutes to rebuild. PyInstaller deletes its whole output folder, so
# these are carried out and back by hand.
RUNTIME_STATE = ("mods", "state.json", "snapshots", "backups", "logs", "cache")
