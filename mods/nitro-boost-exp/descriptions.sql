...
-- "{{percent:comma}}%" instead of "40%" / "10%".
--
...
-- ============= EXPUP_02 (Nitro Boost): 40% -> {{percent:comma}}% =============
UPDATE master_text SET txt = 'Increases EXP gained by {{percent:comma}}%.' || x'0a20'
  WHERE sct = 'SKILL_DESCRIPTION' AND id = 'TXT_SKL_EXPUP_02' AND lang = 'int';

UPDATE master_text SET txt = 'Erhöht erhaltene EP um {{percent:dot}} %.' || x'0a20'
  WHERE sct = 'SKILL_DESCRIPTION' AND id = 'TXT_SKL_EXPUP_02' AND lang = 'deu';

UPDATE master_text SET txt = 'Aumenta un {{percent:dot}}% la EXP ganada.' || x'0a20'
  WHERE sct = 'SKILL_DESCRIPTION' AND id = 'TXT_SKL_EXPUP_02' AND lang = 'esn';

UPDATE master_text SET txt = 'Augmente le taux d''EXP gagnée' || x'0a' || 'de {{percent:dot}} %.' || x'0a20'
  WHERE sct = 'SKILL_DESCRIPTION' AND id = 'TXT_SKL_EXPUP_02' AND lang = 'fra';

UPDATE master_text SET txt = 'Aumenta del {{percent:dot}}% l''ESP ottenuta.' || x'0a20'
  WHERE sct = 'SKILL_DESCRIPTION' AND id = 'TXT_SKL_EXPUP_02' AND lang = 'ita';

UPDATE master_text SET txt = 'Aumenta o ganho de EXP em {{percent:dot}}%.' || x'0a20'
  WHERE sct = 'SKILL_DESCRIPTION' AND id = 'TXT_SKL_EXPUP_02' AND lang = 'ptb';

-- ============= EXPUP_01 (Turbo-charged Engine): 10% -> {{percent:comma}}% =============
UPDATE master_text SET txt = 'Increases EXP gained by {{percent:comma}}%.' || x'0a20'
...
--   ORDER BY id, lang;
--
-- Expected: every row reads "{{percent:comma}}%" instead of "10%" or "40%".
