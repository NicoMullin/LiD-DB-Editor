...
Every weapon lasts *your chosen number of* times as long before it breaks.
...
highest-durability weapon in the game would land 100x past anything this
field has ever actually held; x10 stays a lot closer to ground the game is
known to handle.

## What it changes

`master_part.dur` on every weapon (385 rows).

Replaces `weapon-durability-2x` and `-5x`.
